...
# pode importar essa classe em outro codigo
__all__ = ["ArcBall"]
...
if __name__=="__main__":
    ...
    def mousepress (button, state, x, y):
        if button == GLUT_LEFT_BUTTON:
            if state == GLUT_DOWN:
                global arcball
                arcball = ArcBall ((width/2,height/2,0), width/2) # width/2 eh o raio do arcball
                # considere que exista uma bola dentro do cubo com centro width/2, height/2 e raio width/2
                global startx, starty
                startx, starty = x,y
                glutMotionFunc (rotatecallback) # chama a funcao de rotacionar
            else:
                glutMotionFunc (None)
